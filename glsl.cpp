...
glShader* glShaderManager::loadfromFile(char* vertexFile, char* fragmentFile) 
{
   ...
   { 
     MessageBox(NULL, "Cannot load vertex shader", "SHADER LOAD", MB_OK);
  ...
  {
    MessageBox(NULL, "Cannot load fragment shader", "SHADER LOAD", MB_OK);
  ...
  if (!tVertexShader->compile())
  {
      ...
      MessageBox(NULL, tVertexShader->getCompilerLog(), "COMPILE ERROR - FRAGMENT", MB_OK);
      delete o;
      delete tVertexShader;
      delete tFragmentShader;
      return 0;
  ...
  if (!tFragmentShader->compile())
  {
     ...
     MessageBox(NULL, tFragmentShader->getCompilerLog(), "COMPILE ERROR - FRAGMENT", MB_OK);
  ...
  {
     cout << "**LINKER ERROR\n";
     MessageBox(NULL, o->getLinkerLog(), "LINK ERROR", MB_OK);
